...
function render(){document.getElementById("day").textContent=day;document.getElementById("hp").textContent=hp;document.getElementById("warm").textContent=warm;const story=document.getElementById("story"),choices=document.getElementById("choices");choices.innerHTML="";if(hp<=0||warm<=0){story.textContent="你没能撑过这次穿越。结局：等待救援。";choices.innerHTML="<button onclick='restart()'>重新挑战</button>";return}if(day>events.length){story.textContent="你成功抵达安全点。评级："+(hp+warm>140?"稳健领队":"极限幸存者");choices.innerHTML="<button onclick='restart()'>再来一次</button>";return}const e=events[day-1];story.textContent=e[0];e[1].forEach((c,i)=>{const b=document.createElement("button");b.textContent=c;b.onclick=()=>apply(i);choices.appendChild(b)})}
...
function tap(){count++;document.getElementById("count").textContent=count;document.getElementById("mood").textContent=moods[Math.min(3,Math.floor(count/5))];main.style.transform="scale("+(1+Math.min(.3,count/40))+")";pad.style.background="hsl("+(130+count*24)%360+" 55% 92%)";navigator.vibrate?.([10,20,10])}
...
function repairHtml(html: string, title: string): string {
  ...
  return shell(
    ...
    `let score=0;function render(){document.getElementById("score").textContent=score;if(score>=10)alert("胜利")}document.getElementById("hit").onclick=()=>{score++;navigator.vibrate?.(10);render()};document.getElementById("restart").onclick=()=>{score=0;render()};render();`
  );
}
